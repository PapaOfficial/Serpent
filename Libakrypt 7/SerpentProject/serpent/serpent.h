...
static int ak_serpent_delete_keys(ak_skey skey){
    ...
    if( skey->data != NULL ) {
     //теперь очистка и освобождение памяти
      if(( error = ak_ptr_wipe( skey->data, 544, &skey->generator, ak_true )) != ak_error_ok ) {
...
static int ak_serpent_schedule_keys(ak_skey skey){
    ...
    if(( skey->data = ak_libakrypt_aligned_malloc(544)) == NULL )
    ...
    ak_uint32 w[140];
    ak_uint32* lkey = (ak_uint32*)skey->key.data;
    ak_uint32* rkey = (ak_uint32*)skey->mask.data;
...
int ak_bckey_context_create_serpent(ak_bckey bkey){
    int error = ak_error_ok;
